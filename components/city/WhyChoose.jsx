...
import { BadgeCheck, Zap, Volume2, Box, Wrench, Settings, Headset, IndianRupee } from 'lucide-react';
        ...
        <section className="bg-gradient-to-b from-slate-50 to-white py-7 md:pt-8">
            <div className="mx-auto max-w-7xl px-5">
                ...
                <div className="mx-auto max-w-4xl text-center">
                    <span className="inline-block rounded-full bg-blue-100 px-5 py-2 text-sm font-semibold tracking-widest text-blue-700 uppercase">
                        Why Choose EutAir
                    </span>

                    <h2 className="mt-5 text-2xl font-bold text-slate-900 md:text-5xl">
                        Why Choose{' '}
                    ...
                    <p className="mt-6 text-lg text-slate-600 md:leading-8">
                        At <strong>EutAir</strong>, we understand that every industry has unique compressed air requirements. That's why we provide customized
                        compressor solutions, expert technical guidance, and dependable after-sales support to keep your operations running efficiently.
                    </p>
                </div>

                {/* Content */}
                <div className="mt-6 grid grid-cols-2 items-center gap-4 md:mt-16 md:gap-7 lg:grid-cols-3">
                    {/* Left Image */}
                    ...
                    {features.map((item, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-2 hover:border-blue-500 hover:shadow-xl md:p-5"
                            >
                                ...
                                <p className="mt-3 text-sm text-slate-600 md:leading-6">{item.desc}</p>
                            </div>
                        );
                    })}
                </div>
